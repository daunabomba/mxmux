...
#define ocb_ntz(a) __builtin_ctz((a))
#define ocb_ntz_round(a)                                                                                               \
    ((a) == 0u) ? 0u : (sizeof(unsigned int) * 8 - static_cast<unsigned int>(__builtin_clz((a))) - 1u)
...
static inline unsigned char xtime(unsigned char x) { return ((x << 1) ^ (((x >> 7) & 1) * 0x1b)) & 255; }
...
static void double_arr(unsigned char s[16]) {
    const unsigned char first_bit = ((s[0] >> 7) == 0) ? 1u : 0u;
    for (int i = 0; i < 15; i++) {
        s[i] &= 127;
        s[i] <<= 1;
        s[i] |= s[i + 1] >> 7;
...
void ocb_encrypt(const unsigned char *__restrict key, const unsigned char *__restrict nonce,
                 std::size_t const nonce_length, const unsigned char *__restrict message, std::size_t message_length,
                 unsigned char *out) {
    const unsigned int m = (message_length & 0xfffffff) / 16;
    const unsigned int l_length = (ocb_ntz_round(m) + 1u);
    unsigned char **l = new unsigned char *[l_length];
    for (unsigned int i = 0; i < l_length; ++i) {
        l[i] = new unsigned char[16];
    }
    ...
    const unsigned int p_asterisk_length = (message_length % 16);
    const unsigned int full_block_length = (message_length & 0xfffffff) ^ p_asterisk_length;
    ...
    for (unsigned int i = 0; i < l_length; ++i) {
        delete[] l[i]; // delete each dynamically allocated array of unsigned char[16]
    }
    delete[] l;  // delete the array of pointers
    l = nullptr; // good practice to avoid dangling pointer
}

bool ocb_decrypt(const unsigned char *__restrict key, const unsigned char *__restrict nonce,
                 std::size_t const nonce_length, const unsigned char *__restrict encrypted,
                 std::size_t const encrypted_length, unsigned char *__restrict out) {
    const unsigned int m = (encrypted_length & 0xfffffff) / 16;
    ...
    const unsigned int c_asterisk_length = (encrypted_length % 16);
    const unsigned int full_block_length = (encrypted_length & 0xfffffff) ^ c_asterisk_length;
